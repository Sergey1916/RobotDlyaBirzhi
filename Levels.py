...
extremes.columns = extremes.columns.str.lower()  # нормализуем имена колоно

print(f"Загружено экстремумов: {len(extremes)}")

# Параметры
pg_cur.execute("SELECT start_timestamp FROM bf_algo_sr2_ethusd_params WHERE id = 1")
start_ts = pg_cur.fetchone()[0]

# Определяем ближайший экстремум по времени
...
# Определяем экстремумы ДО точки старта
ext_before = extremes[extremes["timestamp"] <= start_ts]
if len(ext_before) < 2:
    print("Недостаточно экстремумов до старта, берём первые 5.")
...
print(f"Первый тренд: {trend_direction.upper()}, уровень: {level_type}")
...
print(f"Создан первый уровень: ex={base_ex['ex']:.2f}, top={current_level['top']:.2f}, bottom={current_level['bottom']:.2f}")

previous_ex = base_ex

# Основной цикл по экстремумам
for i, ex in extremes.iterrows():
    ...
    if shift_or_transfer:
        ...
        print(f"{shift_or_transfer.upper()} уровня: ex={ex['ex']:.2f}, дата={ex['datetime']}")
...
print(f"Уровни успешно сохранены в PostgreSQL: {len(levels)}")
